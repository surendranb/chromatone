...
object NoiseGenerator {
    ...
    /**
     * Generate a buffer of azure noise samples (steeper differentiation)
     */
    fun generateAzureNoise(numSamples: Int): ShortArray {
        val buffer = ShortArray(numSamples)
        var last = 0.0
        var lastBlue = 0.0
        for (i in buffer.indices) {
            val white = Random.nextDouble(-1.0, 1.0)
            val blue = white - last
            val azure = blue - lastBlue
            last = white
            lastBlue = blue
            buffer[i] = (azure * Short.MAX_VALUE).toInt().toShort()
        }
        return buffer
    }
    ...
    fun generateVioletNoise(numSamples: Int): ShortArray {
        ...
        for (i in buffer.indices) {
            ...
            last = white
            lastBlue = blue
            // Violet is even higher frequency, we'll keep it consistent with the math
    ...
    /**
     * Generate a buffer of grey noise samples (Apprx. inverted equal-loudness)
     */
    fun generateGreyNoise(numSamples: Int): ShortArray {
        // Simple approximation: mix of pink and violet noise to flatten perceived loudness
        val buffer = ShortArray(numSamples)
        val rows = 16
        val random = Random.Default
        val whitePool = DoubleArray(rows)
        var lastBlue = 0.0
        for (i in buffer.indices) {
            val k = random.nextInt(rows)
            whitePool[k] = random.nextDouble(-1.0, 1.0)
            val pink = whitePool.sum() / rows
            val white = random.nextDouble(-1.0, 1.0)
            val blue = white - lastBlue
            lastBlue = white
            // Mix: Grey is perceptually flat by being more 'mid' focused
            val grey = (pink + blue) * 0.5
            buffer[i] = (grey * Short.MAX_VALUE).toInt().toShort()
        }
        return buffer
    }

    /**
     * Generate a buffer of red noise samples (Heavy Brown/Red noise)
     */
    fun generateRedNoise(numSamples: Int): ShortArray {
        val buffer = ShortArray(numSamples)
        var last = 0.0
        for (i in buffer.indices) {
            val white = Random.nextDouble(-1.0, 1.0)
            // Heavier integration than Brown (0.05 step instead of 0.02)
            last = (last + (0.05 * white)).coerceIn(-1.0, 1.0)
            buffer[i] = (last * Short.MAX_VALUE).toInt().toShort()
        }
        return buffer
    }
    ...
    fun getNoiseBuffer(type: NoiseType, numSamples: Int): ShortArray = when (type) {
        ...
        NoiseType.Green -> generateGreenNoise(numSamples, 44100)
        NoiseType.Blue -> generateBlueNoise(numSamples)
        NoiseType.Violet -> generateVioletNoise(numSamples)
        NoiseType.Grey -> generateGreyNoise(numSamples)
        NoiseType.Azure -> generateAzureNoise(numSamples)
        NoiseType.Red -> generateRedNoise(numSamples)
    }
}
